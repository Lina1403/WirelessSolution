...
public class AddUser {
    ...
    @FXML
    private ChoiceBox<User.Role> roleChoiceBox;

    @FXML
    private HBox conciergeAttributes;

    @FXML
    private TextField serviceField;

    @FXML
    private TextField horraireServiceField;

    @FXML
    private HBox residentAttributes;

    @FXML
    private Date dateOfArrivalField;

    @FXML
    private TextField numberOfEmergencyField;
    ...
    private void handleAddUserButtonClick() {
        try {
            ...
            User.Role role = roleChoiceBox.getValue();
    ...
    private void initialize() throws SQLException {
        ...
        roleChoiceBox.getItems().addAll(User.Role.values());

        // Set up event handlers for role choice box
        roleChoiceBox.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue == User.Role.CONCIERGE) {
                conciergeAttributes.setVisible(true);
                residentAttributes.setVisible(false);
            } else if (newValue == User.Role.RESIDENT) {
                conciergeAttributes.setVisible(false);
                residentAttributes.setVisible(true);
            } else {
                conciergeAttributes.setVisible(false);
                residentAttributes.setVisible(false);
            }
